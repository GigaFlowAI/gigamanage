 ...
 * end up reading `~/.config/gmux/config.json` — and then pass or fail
 * depending on whether the person running it happens to have run `gmux setup`.
 *
 ...
 * cache is the half gmux *writes* — index and summaries. Left unset, a
 * test that touches the index path clobbers the developer's real
 * `~/.cache/gmux` rather than merely reading it.
 */
...
beforeAll(async () => {
  configRoot = await mkdtemp(join(tmpdir(), "gmux-config-"));
  cacheRoot = await mkdtemp(join(tmpdir(), "gmux-cache-"));
